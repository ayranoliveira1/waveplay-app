...
| 5.8 | **Cookies inseguros** | Falta de Secure, HttpOnly, SameSite em cookies | Refresh token cookie: httpOnly=true, secure=true (producao), sameSite='strict', path='/auth/refresh'. Nunca enviar cookie sem flags de seguranca | Alta |
...
| 19.14 | **Password Input sem mascaramento** | Senha visivel durante digitacao em ambiente compartilhado / captura de tela | `secureTextEntry={true}` por padrao em todos os 3 inputs (senha atual, nova, confirmacao). Toggle "Mostrar/Ocultar" via state local. `autoCapitalize="none"` + `autoCorrect={false}` em todos. `textContentType="password"` (atual) e `"newPassword"` (nova/confirmacao) para hint correto ao keychain do SO | Alta |
| 19.15 | **Submit Spam em Change Password** | User toca varias vezes em "Alterar senha" e dispara N requests | Botao com `isLoading={isLoading}` (disabled durante request). Backend tem rate-limit 5/60s como fallback. `apiError` reseta a cada `onChangeText` para nao acumular feedback antigo | Media |
| 19.16 | **SignOut Forcado Apos Change Password** | App desloga sessao atual desnecessariamente apos sucesso | **Nao** chamar `signOut()` em `ChangePasswordScreen.onSubmit` apos sucesso. Backend ja revogou as outras sessoes via `revokeAllByUserId`; sessao atual permanece valida (refresh do device atual e o ultimo a ser invalidado pelo proprio refresh). Apos `Alert.alert`, apenas `navigation.goBack()` | Media |
| 19.17 | **Logando Valor de Senha** | `console.log(data)` ou Sentry breadcrumb com payload do form vaza senha | Nunca logar `data`, `currentPassword`, `newPassword`. No `onSubmit` da `ChangePasswordScreen`, apenas extrair os valores e enviar via `auth.changePassword(...)`. Em erro, logar so a mensagem retornada (`response.error?.[0]?.message`), nunca o payload do form | Alta |
| 19.18 | **Confiar em `confirmPassword` no Backend** | Backend espera 3 campos e atacante manipula client pra mandar mismatch | Por design, `confirmPassword` e validacao client-side de UX (anti-typo). Backend recebe so `{ currentPassword, newPassword }` no payload PATCH — atacante que controla o client nao tem vetor de ataque | Baixa |
